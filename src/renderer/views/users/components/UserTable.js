...
class UserTable {
    ...
    render(users, currentUser) {
        ...
        if (users.length === 0) {
            tbody.innerHTML = `<tr><td colspan="6" class="text-center p-lg text-muted" style="background: #f9f9f9;">No users found in this view.</td></tr>`;
            return;
        }
        ...
        tbody.innerHTML = users.map(u => {
            ...
            // Status Badge Styling (Added a specific badge for "Trashed")
            let statusStyle = '';
            let statusText = u.status.charAt(0).toUpperCase() + u.status.slice(1);
            ...
            <tr class="hover:bg-neutral-50 transition-colors" style="border-bottom: 1px solid #f0f0f1; font-size: 13px;">
                <td style="padding: 12px;">
                    <div style="color: #2271b1; font-weight: 600; text-decoration: ${isDeleted ? 'line-through' : 'none'}; opacity: ${isDeleted ? '0.6' : '1'};">${u.name || 'No Name'}</div>
                </td>
                <td style="padding: 12px; color: #50575e; opacity: ${isDeleted ? '0.6' : '1'};">${u.email}</td>
                <td style="padding: 12px;">
                ...
                <td style="padding: 12px;" class="text-right">
                    ${currentUser.id !== u.id ? `
                        <div class="flex gap-xs justify-end items-center">
                            ${isDeleted ? `
                                <button class="btn btn-sm btn-ghost btn-restore-user" data-id="${u.id}" title="Restore User" style="padding: 4px 8px; color: #2271b1; border-color: transparent;">
                                    <i class="fa-solid fa-trash-arrow-up"></i>
                                </button>
                                <button class="btn btn-sm btn-ghost btn-perm-delete-user" data-id="${u.id}" data-name="${u.name}" title="Destroy Permanently" style="padding: 4px 8px; color: #b91c1c; border-color: transparent;">
                            ...
                            ` : `
                                ...
                                <button class="btn btn-sm btn-ghost btn-delete-user" data-id="${u.id}" data-name="${u.name}" title="Move to Trash" style="padding: 4px 8px; color: #d63638; border-color: transparent;">
    ...
    attachRowEvents() {
        ...
        document.querySelectorAll('.btn-perm-delete-user').forEach(btn => btn.addEventListener('click', () => this.modals.handlePermanentDelete(btn.dataset.id, btn.dataset.name)));
